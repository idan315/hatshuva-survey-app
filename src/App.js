  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <div class="footer">
          <p>
            אתם משתתפים בניסוי שמטרתו למצוא את מילת התואר המחמיאה ביותר בעברית.{" "}
            <strong>
              שימו לב, מספר השאלות בסקר הוא לא מוגבל: תמשיכו לקבל עוד ועוד שאלות
              עד שתחליטו להפסיק
            </strong>
            . להסברים נוספים, אתם מוזמנים להאזין לפרק "
            <a href="https://www.osimhistoria.com/theanswer/ep121-sabbaba">
              אחלה או סבבה?
            </a>
            " של הפודקאסט{" "}
            <a
              href="https://www.osimhistoria.com/theanswer"
              target="_blank"
          ...
          <p>
            קודד: עידן זיירמן (וגם לו יש פודקאסטים! האזינו ל
            <a
              rel="noreferrer noopener"
              href="https://tzofim.podbean.com/"
              target="_blank"
            >
              "צופים בין כוכבים"
            </a>{" "}
            שבכל שבוע נערך בו דיון קצר ומשעשע על פרק ״מסע בין כוכבים״ לפי הסדר,
            או{" "}
            <a
              rel="noreferrer noopener"
              href="https://www.gamepad.co.il/"
              target="_blank"
            >
              ״גיימפוד״
            </a>{" "}
            - פודקאסט משחקי המחשב והקונסולות הוותיק ביותר בעברית).
          </p>
          <p>
            נמאס לכם רק להחמיא? נסו את הניסוי המקביל -{" "}
            <a
              href="https://magarooah.herokuapp.com/"
              rel="noreferrer noopener"
            >
              מה גרוע יותר
            </a>
...
class WordsComponent extends React.Component {
  ...
  async retrieveNewWords() {
    this.setState({
      words: await getWordPairFromServer(),
      nounIndex: getRandomNounIndex(),
    });
  }
  ...
  render() {
    return (
      <div class="selection">
        <WordComponent
          winner={this.state.words[0]}
          loser={this.state.words[1]}
          uuid={this.state.words[2]}
          nounIndex={this.state.nounIndex}
          retrieveWordsCallback={this.retrieveNewWords}
        />
        <WordComponent
          winner={this.state.words[1]}
          loser={this.state.words[0]}
          uuid={this.state.words[2]}
          nounIndex={this.state.nounIndex}
          retrieveWordsCallback={this.retrieveNewWords}
        />
...
class WordComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      loser: props.loser,
      uuid: props.uuid,
      nounIndex: props.nounIndex,
      retrieveWordsCallback: props.retrieveWordsCallback,
    };
  }
  ...
  componentDidMount() {
    this.setState({
      ...
      nounIndex: this.props.nounIndex,
    });
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <div class="title">
          ה{noun} {gender === Gender.MALE ? "הזה" : "הזו"}{" "}
        </div>
        <div class="word">
          {gender === Gender.MALE
            ? this.props.winner
            : this.convertToFemale(this.props.winner)}
        </div>
